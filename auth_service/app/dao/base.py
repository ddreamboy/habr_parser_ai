...
class BaseDAO(Generic[T]):
    ...
    async def find_all_with_relations(self, *relations):
        """Поиск всех записей с загрузкой связанных объектов"""
        logger.info(f"Поиск всех записей {self.model.__name__} с загрузкой связей")
        try:
            ...
            logger.info(f"Найдено {len(records)} записей с загруженными связями.")
            return records
        except SQLAlchemyError as e:
            logger.error(f"Ошибка при поиске записей с связями: {e}")
            raise

    async def find_one_with_relations(self, relations: list, **filters):
        """
        Поиск одной записи по фильтрам с загрузкой указанных связанных объектов
        """
        logger.info(
            f"Поиск одной записи {self.model.__name__} по фильтрам {filters} с загрузкой связей"
        )
        ...
        try:
            ...
            result = await self._session.execute(query)
            record = result.scalar_one_or_none()
            log_message = f"Запись {'найдена' if record else 'не найдена'} по фильтрам {filters} с загруженными связями."
            logger.info(log_message)
            return record
        except SQLAlchemyError as e:
            logger.error(
                f"Ошибка при поиске записи с связями по фильтрам {filters}: {e}"
            )
            raise
